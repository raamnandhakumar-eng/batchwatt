...
(function (root) {
  ...
  function createEnergyPlan(input) {
    ...
    const slots = Array.from({ length: (end - start) / SLOT_MINUTES }, (_, i) => {
      ...
      const imported = importedIntervals.get(time);
    ...
    function schedule(mode) {
      const profile = slots.map(s => ({ ...s, kw: Number(s.baseKw), jobs: [] }));
    ...
    return { schemaVersion: '2.2', generatedAt: new Date().toISOString(), factory: input.factory || 'My factory', shift, energy,
      allocations, baseline, proposed, decisions, warnings,
      energyInput: { intervalDataLoaded: importedIntervals.size > 0, intervalsLoaded: importedIntervals.size, intervalsUsed, shiftIntervals: slots.length },
      ...
      assumptions: ['15-minute load model. Imported interval baseline load overrides the configured base load at matching times; missing intervals fall back to the configured base load.',
        'Imported interval tariff rates override the configured tariff at matching times; missing interval rates use the configured peak/off-peak tariff window.',
        'Imported facility kW should represent background/baseline load before BatchWatt adds the schedulable production-line loads to avoid double counting.',
