  ...
  function createEnergyPlan(input) {
    ...
    function schedule(mode) {
      ...
      for (const order of allocations) {
        ...
        const later = allocations.slice(position + 1).filter(o => o.lineId === line.id && o.produce > 0 && !o.packagingShort);
    ...
    if (proposed.unscheduled.length) warnings.push(`${proposed.unscheduled.length} order(s) need packaging or more shift capacity.`);
    ...
    return { schemaVersion: '2.0', generatedAt: new Date().toISOString(), factory: input.factory || 'My factory', shift, energy,
      allocations, baseline, proposed, warnings,
      ...
      assumptions: ['15-minute constant-load model, including base load during the shift. Short runs and changeovers round up to a full interval.',
        ...
        'Raw materials, machine dependencies and detailed process constraints require supervisor review. This is a scheduling heuristic, not a guaranteed optimum.'] };
  }
