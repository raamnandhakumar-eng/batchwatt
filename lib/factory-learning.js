...
(function(root){
  ...
  const changed=(a,b)=>['id','externalId','customer','productId','product','qty','due','priority'].some(k=>String(a?.[k]??'')!==String(b?.[k]??''));
  ...
  function merge(state,orders=[],products=[],capturedAt=new Date().toISOString()){
    ...
    for(const o of orders||[]){
      ...
      const key=orderKey(o),obs={key,id:norm(o.id),externalId:norm(o.externalId),customer:norm(o.customer)||'Unknown customer',productId:norm(o.productId),product:productName(o.productId,products),qty:safeNum(o.qty),due:norm(o.due),priority:norm(o.priority)||'Standard'};
      if(byKey.has(key)){
        const i=byKey.get(key),prior=next.observations[i];
        if(changed(prior,obs)){next.observations[i]={...prior,...obs,lastSeenAt:capturedAt};updated++;}
      }else{
